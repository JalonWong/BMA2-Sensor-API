...
int main(void)
{
    ...
    rslt = bma2_enable_interrupt(BMA2_INT_EN_FIFO_FULL, &dev);
    ...
    rslt = bma2_set_int_mapping(BMA2_INT_MAP, BMA2_INT1_MAP_FIFO_FULL, &dev);
    ...
    while (try < 10)
    {
        ...
        if (int_status.int_status_1 & BMA2_INT_1_ASSERTED_FIFO_FULL)
        {
            printf("\nIteration: %d\n", try);
